 ...
 *
 * <p>
 * Supports toggle-based reaction operations on news posts, including finding
 * an existing reaction for a user-post pair and computing per-type reaction
 * counts
...
public interface PostReactionRepo extends JpaRepository<PostReaction, Long> {
    ...
    /**
     * Finds an existing reaction submitted by a specific account on a specific
     * post.
     *
     * <p>
     * This is the primary lookup used by the toggle reaction service to decide
     * whether to create, update, or delete a reaction.
     *
     * @param postId    the ID of the
     *                  {@link com.rikkeisoft.backend.model.entity.ForumPost}
     *                  that was reacted to.
     * @param accountId the UUID string ID of the
     *                  {@link com.rikkeisoft.backend.model.entity.Account}
     *                  that submitted the reaction.
     * @return an {@link Optional} containing the {@link PostReaction} if one
     *         exists;
     ...
     *
     * <p>
     * Returns a list of Object arrays where each element is
     * {@code [reactionType, count]},
    ...
    // Xóa trực tiếp mà không cần fetch trước
    void deleteByPostIdAndAccountId(Long postId, String accountId);

    // Check existence
    boolean existsByPostIdAndAccountId(Long postId, String accountId);

    // Lấy tất cả reactions của 1 post (để hiển thị)
