    ...
    <>
      {isOpen && (
        <button
          ...
          onClick={onClose}
          aria-label={t("filters.actions.close")}
        />
      )}
      <aside
        className={`fixed inset-y-0 right-0 z-50 w-full max-w-md border-l border-slate-200 bg-white shadow-xl transition-transform duration-300 dark:border-slate-800 dark:bg-slate-950 ${
      ...
      >
        ...
        <div className="space-y-6 overflow-y-auto px-6 py-6">
                ...
                >
                  <input
                    ...
                    checked={statusValues.includes(
                      option.value as CandidateFilterStatus
                    )}
                    onChange={() =>
                      toggleMultiSelect("statuses", option.value)
                    }
                    className="h-4 w-4"
                  />
              ...
              {CANDIDATE_LOCATION_OPTIONS.map((option) => (
                ...
                >
                  <input
                    ...
                    onChange={() =>
                      toggleMultiSelect("locations", option.value)
                    }
                    className="h-4 w-4"
                  />
                  <span>{t(option.labelKey)}</span>
              ...
              {CANDIDATE_INDUSTRY_OPTIONS.map((option) => (
                ...
                >
                  <input
                    ...
                    onChange={() =>
                      toggleMultiSelect("industries", option.value)
                    }
                    className="h-4 w-4"
                  />
                  <span>{t(option.labelKey)}</span>
          ...
          <section>
            <LabelText className="mb-3 block">
              {t("filters.sections.education")}
            </LabelText>
            <p className="mb-2 text-xs text-slate-500">
              {t("filters.educationNote")}
            </p>
            <div className="space-y-2">
              {CANDIDATE_EDUCATION_OPTIONS.map((option) => (
                ...
                >
                  <input
                    ...
                    onChange={() =>
                      toggleMultiSelect("educationLevels", option.value)
                    }
                    className="h-4 w-4"
                  />
                  <span>{t(option.labelKey)}</span>
